...
function makeApi(initialTheme: 'light' | 'dark' = 'light') {
  ...
  // 사이클 12: 테마 팩 관련 API mock — 빈 목록 반환, no-op dispose
  const themePackList = vi.fn().mockResolvedValue([]);
  const onThemePackSetActive = vi.fn().mockReturnValue(() => undefined);
  const onThemePackListChanged = vi.fn().mockReturnValue(() => undefined);

  return { getTheme, watchTheme, watchDisposes, themePackList, onThemePackSetActive, onThemePackListChanged };
}
...
function installMockApi(api: ReturnType<typeof makeApi>) {
  Object.defineProperty(window, 'api', {
    value: {
      ...
      themePackList: api.themePackList,
      onThemePackSetActive: api.onThemePackSetActive,
      onThemePackListChanged: api.onThemePackListChanged,
    },
    writable: true,
    configurable: true,
  });
