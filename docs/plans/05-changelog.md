...
| 일자           | 리뷰어        | 항목                                                 | 반영 결과                                                                                                                                            |
| -------------- | ------------- | ---------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2026-05-01     | doc-writer    | **사이클 11b 완료 마킹** | index chunk 881KB / -92KB(AC6 ≥30KB 달성) + Cask 자체 tap(`bluemirr5/homebrew-md-dolphin`) + 거버넌스 4종 + README 한·영(7섹션, Gatekeeper 5단계 + 번들 솔직 표기) + Phase 2 체크리스트(5종 게이트 임계치 명시) + v0.11.1 chunk split 릴리스. 사용자 작업 인계: v0.11.1 tag 발사 / Cask sha256 실측 hex 치환 / 스크린샷 5장 캡처 |
...
| **2026-05-01** | **doc-writer** | **사이클 8 완료 마킹 + 마스터 플랜 갱신** | OutlineExtractor TS 분리(adapter 6개 헬퍼 → outline-extractor.ts) + 목차 사이드바(SidebarView/Toggle/useScrollSpy/scrollToAnchor/sidebar-store) + ⌘1/⌘2. 255 tests (+31). P8 라운드 8건 흡수(P8-1 옵션 A 모듈 최상위 싱글턴, P8-2~P8-8 권고 반영). CR8-5개 Major 수정(useEffect→ref callback, parseMarkdown useMemo, localStorage runtime 검증, transition 제거, anchor section 폴백). 신규 의존성 0건. 회귀 p50 4.39ms (변동 0%). adapter 외부 export 동결 회복(parseMarkdown + getCachedTokens 2개) 유지. **사이클 9 인계 부채 추가 3건**: CR8-6 NodeList 자식 재귀 명시성, CR8-7 useScrollSpy articleRef 의존성 일관성, CR8-8 sidebar reload AC7 단위 테스트 보강. 사이클 7 인계 부채 4건(CR7-5/9/10/11)은 사이클 9 그대로 유지. |
| **2026-05-01** | **doc-writer** | **사이클 9 완료 마킹 + 마스터 플랜 갱신** | 메뉴/줌/PDF + 가상 스크롤(react-virtuoso) + shiki lazy loadLanguage + 벤치 4지표 측정 + 부채 7건(CR7-5/9/10/11 + CR8-6/7/8) 흡수. 332 tests (+77). P9/CR9/CR9.2 라운드 16건 흡수. **사이클 10 인계 부채**: AC3d emacs-lisp 청크 804KB 임계 초과(lazy 로드라 초기 번들 무영향), CR9-S1 bench:cold-start production tree-shake CI 검증, CR9-S2 useScrollSpy ref callback 패턴 리팩터링, AC3b 콜드 스타트·AC3c 메모리 수동 측정. |
| **2026-05-01** | **doc-writer** | **사이클 10 완료 마킹 + 마스터 플랜 갱신** | 에러 UX 5종(permission/encoding/not-markdown/too-large/empty) + ARIA 보강 + i18n(i18next ^26 + react-i18next ^17) + 동적 폰트 스케일 + axe-core dev. 413 tests (+81). P10 9건 + CR10 4건 흡수. 회귀 p50 4.31ms ≤5초 합격, renderer bundle +98.3KB (+50KB 게이트 초과). **사이클 11a 인계 부채 9건**: CR10-1/6/7/8/9/10/11, CR10-12(CR10-5 협력). |
| **2026-05-01** | **doc-writer** | **사이클 11a 완료 마킹 + 마스터 플랜 갱신** | OSS 배포 본진(electron-builder ^26.8.1 + macOS Universal DMG + GitHub Actions release.yml + ci.yml 분리) + 부채 9건 흡수 + P11a 10건 + CR11a 7건 반영. 431 tests (+18: zoom-bridge-cleanup 7 + error-state-aria-i18n 11). 차단 P11a-2(entitlements 키 미참조) 해소. CR11a-3 release-notes.md sed step 추가. **사이클 11b 인계**: v0.11.0 tag 실측, macOS 14/15 수동 검증, DMG ≤130MB, 빌드 시간 ≤30분, renderer +98.3KB 분할(i18next chunk split ≥30KB 11b 즉시 적용 권고). |
| **2026-05-01** | **developer** | **사이클 11b 완료 마킹** | Homebrew Cask(v0.11.1 SHA placeholder) + README 한·영(7섹션, Gatekeeper 5단계, 번들 크기, SHA256) + 거버넌스 4종(LICENSE/CONTRIBUTING.md/CODE_OF_CONDUCT.md 외부 URL 참조/SECURITY.md) + Phase 2 체크리스트(7단계 + 결정 게이트 5종 임계치) + Cask PR 템플릿 + i18next chunk split(index 973KB→881KB, -92KB, AC6 ≥30KB 달성) + release-notes README 링크 치환(v0.11.1 이후 적용). 신규 의존성 0건. **P11b 라운드**: P11b-1~P11b-7 스펙 반영 완료. **사이클 12 인계**: react-i18next dynamic import(효과 <30KB), shiki dual theme 커스텀 JSON, AC3b/c 콜드 스타트·메모리 수동 측정. |
