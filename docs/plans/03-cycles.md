...
| 10  | **✅ 완료 — 에러 UX · 접근성 · i18n** | ErrorState(5종 FileError) + ARIA(role/label/status/dialog) + i18next(ko/en) + 동적 폰트(--font-scale) + axe-core. 413 tests (+81). P10-1~P10-9 9건 흡수. CR10-2/3/4/5 즉시 수정. 회귀 p50 4.31ms ≤5초 합격(-1.8% vs C9). **사이클 11a 인계 부채 9건**: CR10-1(too-large 순서), CR10-6(zoom-bridge dispose), CR10-7(ZOOM_STEP), CR10-8(의존성 v26/v17), CR10-9(a11y i18n), CR10-10(encoding dead), CR10-11(bundle verify), CR10-12(CR10-5 협력), renderer +98.3KB(+50KB 게이트 초과) | `ErrorState`/`LargeFileWarning`, i18n-lookup helper, zoom-bridge, axe.ts, verify-prod-bundle.mjs | 모두    | TDD (에러 분기 로직) |
