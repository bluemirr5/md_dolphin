...
| 9   | **✅ 완료 — 메뉴/줌/PDF + 가상 스크롤 + 성능** | ⌘+/-, ⌘0, macOS 표준 메뉴, 인쇄, PDF 저장. **4.7.3 성능 벤치 표준** 4지표 측정: 렌더 p50/p95, 콜드 스타트, 메모리. **`react-virtuoso` 가상 스크롤 도입**. shiki lazy `loadLanguage` 전환(빌드 청크 700KB+ 해소). 332 tests (+77). P9 라운드 7건 흡수(AC 4지표 분리, 가상화 정합, IPC 4건 whitelist, 부채 4건 parity). CR9 Major 5건 + CR9.2 Critical 1건 수정. **부채 7건 모두 흡수**: CR7-5/9/10/11(Image useMemo, sanitize 격리, O_NOFOLLOW, windowId -1) + CR8-6/7/8(NodeList, useScrollSpy, sidebar AC7). shiki dual theme 커스텀 JSON 검토는 사이클 10으로 인계. | 메뉴 명령 핸들러, zoom/print IPC, 벤치마크 문서(`docs/benchmarks/cycle-09-perf.md`), `react-virtuoso` 통합, 부채 처리 완료. **사이클 10 인계 부채**: AC3d emacs-lisp 청크 804KB(lazy 로드라 초기 번들 무영향), CR9-S1 bench:cold-start production tree-shake CI, CR9-S2 useScrollSpy ref callback 리팩터링, AC3b/c 수동 측정 | 3, 6, 7, 8 | TDD (벤치 + 가상화 어댑터) |
| 10  | **✅ 완료 — 에러 UX · 접근성 · i18n** | ErrorState(5종 FileError) + ARIA(role/label/status/dialog) + i18next(ko/en) + 동적 폰트(--font-scale) + axe-core. 413 tests (+81). P10-1~P10-9 9건 흡수. CR10-2/3/4/5 즉시 수정. 회귀 p50 4.31ms ≤5초 합격(-1.8% vs C9). **사이클 11a 인계 부채 9건**: CR10-1(too-large 순서), CR10-6(zoom-bridge dispose), CR10-7(ZOOM_STEP), CR10-8(의존성 v26/v17), CR10-9(a11y i18n), CR10-10(encoding dead), CR10-11(bundle verify), CR10-12(CR10-5 협력), renderer +98.3KB(+50KB 게이트 초과). **비고**: i18next v26.x / react-i18next v17.x 실설치 채택(스펙 ^23/^14에서 메이저 갱신, CR10-8 처리) | `ErrorState`/`LargeFileWarning`, i18n-lookup helper, zoom-bridge, axe.ts, verify-prod-bundle.mjs | 모두    | TDD (에러 분기 로직) |
| 11a | **✅ 완료 — OSS 배포 빌드 + DMG + Releases** | electron-builder ^26.x + Universal DMG(x64+arm64) + macos-14 release.yml + ci.yml 분리 + Gatekeeper inline 안내 + 부채 9건. 431 tests (+18). P11a 10건 흡수(차단 P11a-2 entitlements 미참조 해소). CR11a 7건 즉시 수정(release-notes.md sed step·io ariaLabel nested·CR10-9 한국어 하드코딩 0건 달성). | electron-builder.yml, release.yml, ci.yml, release-notes.template.md, build/entitlements.mac.plist(미참조 stub), src/shared/zoom.ts. **사이클 11b 인계**: v0.11.0 tag 실측, macOS 14/15 수동 검증, DMG/SHA256 실측 기재, renderer +98.3KB 분할(i18next chunk split ≥30KB 11b 즉시 적용 후보, 미만 사이클 12). | 모두 | 사후 R1/R2 (부채 회귀) |
| 11b | **✅ 완료 — Homebrew + 거버넌스 + Phase 2 전환 준비** | Homebrew Cask(`homebrew-tap/Casks/md-dolphin.rb` v0.11.1 SHA placeholder) + README 한·영(Gatekeeper 안내 5단계·번들 크기 솔직 표기·SHA256 검증) + 거버넌스 4종(LICENSE/CONTRIBUTING.md/CODE_OF_CONDUCT.md/SECURITY.md) + Phase 2 체크리스트 + Cask PR 템플릿 + i18next chunk split(index chunk 973KB → 881KB, -92KB, AC6 ≥30KB 달성) + release-notes README 링크 치환. **Cask `auto_updates: false` + `livecheck` GitHub Releases (P2-11)**. | Cask 자체 tap / 거버넌스 4종 / README 한·영 / Phase 2 체크리스트 / i18next chunk split / v0.11.0 실측 / v0.11.1 chunk split 릴리스 | 11a | 사후 R1 (i18n chunk 생성 확인) | **사용자 작업 인계**: v0.11.1 tag 발사(`git tag v0.11.1 && git push --tags`) / Cask sha256 hex 실측 치환 / 스크린샷 5장 캡처 |
