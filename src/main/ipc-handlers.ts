 ...
 * @param themePackService 테마 팩 디스커버리 서비스 (사이클 12)
...
export function registerIpcHandlers(
  ...
  if (themePackService) {
    // api:themePackList — 빌트인 + 사용자 팩 목록 반환
    ipcMain.handle(API_THEME_PACK_LIST, async () => {
      return themePackService.listPacks();
    });

    // api:themePackGet — id로 팩 조회
    ipcMain.handle(API_THEME_PACK_GET, async (_event, id: string) => {
      return themePackService.getPack(id);
    });

    // api:themePackRevealFolder — themes 폴더 Finder에서 열기 + 목록 변경 push
    ipcMain.handle(API_THEME_PACK_REVEAL_FOLDER, async () => {
      await themePackService.revealFolder();
      // reveal 후 즉시 목록 변경 알림 push
      const packs = await themePackService.listPacks();
      broadcastToAllWindows(API_THEME_PACK_LIST_CHANGED, packs);
    });

    // api:themePackMenuSync — renderer가 활성 팩 id를 main에 알림 (메뉴 radio 갱신용)
    ipcMain.on(API_THEME_PACK_MENU_SYNC, (_event, _activeId: string) => {
      // 메뉴 재빌드는 index.ts에서 installMenu 재호출로 처리
      // 현재는 noop — 추후 메뉴 동적 갱신 시 구현
    });
  }
  ...
  return () => {
    ...
    // 사이클 12 테마 팩 채널 정리
    if (themePackService) {
      ipcMain.removeHandler(API_THEME_PACK_LIST);
      ipcMain.removeHandler(API_THEME_PACK_GET);
...
/**
 * 살아있는 모든 BrowserWindow에 임의 채널+데이터를 broadcast한다.
 */
