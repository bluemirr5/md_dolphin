  ...
  return (
    <Virtuoso
      ...
      itemContent={(index, item) => (
        <div className="md-content__item">{renderItem(item, index)}</div>
      )}
      components={{ Scroller, Header: TopSpacer, Footer: BottomSpacer }}
      rangeChanged={handleRangeChanged}
      // 평균 블록 높이 추정값 — 초기 totalHeight 추정 정확도 ↑ (보정 점프 완화)
      defaultItemHeight={120}
      // viewport 밖 800px까지 미리 mount → ResizeObserver 실측을 선행해
      // 휠 진행 중 totalHeight 보정으로 인한 스크롤 점프를 거의 제거
      increaseViewportBy={{ top: 800, bottom: 800 }}
    />
  );
}

// 상하 32px 여백 — react-virtuoso의 Viewport(`position: absolute`)가
// article의 padding을 덮어버리므로 Header/Footer 컴포넌트로 spacer를 삽입한다.
function TopSpacer(): JSX.Element {
  return <div className="md-content__spacer md-content__spacer--top" />;
}
function BottomSpacer(): JSX.Element {
  return <div className="md-content__spacer md-content__spacer--bottom" />;
}
